        ...
        for i in range(len(room.get_exits())):
            for j in range(len(next_room.get_exits())):
                if next_room.id not in directions:
                    directions[next_room.id] = {
                        next_room.get_exits()[j]: '?'
                        for j in range(len(next_room.get_exits()))
                    }
                if room.get_room_in_direction(
                        room.get_exits()[i]) == next_room:
                    directions[room.id].update(
                        {room.get_exits()[i]: next_room.id})
                    directions[next_room.id].update(
                        {opposite_lookup.get(room.get_exits()[i]): room.id})
    else:
        directions[room.id] = {
            room.get_exits()[i]: '?'
            for i in range(len(room.get_exits()))
...
def walk_in_straight_line(direction):
    ...
    while directions[player.current_room.id][direction] == '?':
        update_map(player.current_room,
                   player.current_room.get_room_in_direction(direction))
        if directions[player.current_room.get_room_in_direction(
...
for i in range(len(player.current_room.get_exits())):
    if directions[player.current_room.id][player.current_room.get_exits()
                                          [i]] == '?':
